import type { deleteRequestItem, passRequestItem, responseItem } from '@/models/base'
...
export async function dismissMembersByTeamId(data: deleteRequestItem): Promise<responseItem<boolean>> {
  const response = await fetch(baseUrl + '/member/dismiss', {
    method: 'POST',
    body: JSON.stringify(data),
    headers: {
      'Content-Type': 'application/json'
...
export async function handleDismiss(data: deleteRequestItem): Promise<responseItem<boolean>> {
  const response = await fetch('/api/member/dismiss', {
    method: 'POST',
    body: JSON.stringify(data),
    headers: {
      'Content-Type': 'application/json'
